...
contract ProlendInterestRateTest is Test {
    ...
    function testInterestRateConstants() public {
        ...
        (uint256 minAPR, uint256 vertexAPR, uint256 maxAPR, uint256 vertexUtil) = interestRate.getRateCurveAPR();
        
        assertEq(minAPR, 200, "Min APR should be 2% (200 basis points)");
        assertEq(vertexAPR, 1000, "Vertex APR should be 10% (1000 basis points)"); 
    ...
    function testUtilizationCalculation() public {
        ...
        assertEq(interestRate.calculateUtilization(0, 100 ether), 0, "0% utilization");
        assertEq(interestRate.calculateUtilization(50 ether, 100 ether), 50000, "50% utilization");
        assertEq(interestRate.calculateUtilization(80 ether, 100 ether), 80000, "80% utilization");
        assertEq(interestRate.calculateUtilization(100 ether, 100 ether), 100000, "100% utilization");
        
        // Test edge case: more borrowed than available (should cap at 100%)
        assertEq(interestRate.calculateUtilization(150 ether, 100 ether), 100000, "Over 100% utilization capped");
        
        // Test zero total assets
        assertEq(interestRate.calculateUtilization(10 ether, 0), 0, "Zero assets should give 0% utilization");
    }
    ...
    function testInterestRateBeforeKink() public {
        ...
        assertEq(rate0, interestRate.MIN_RATE(), "0% utilization should give MIN_RATE");
        assertEq(rateKink, interestRate.VERTEX_RATE(), "80% utilization should give VERTEX_RATE");
        
        // 40% utilization should be halfway between min and vertex rates
        uint256 expectedHalfRate = interestRate.MIN_RATE() + (interestRate.VERTEX_RATE() - interestRate.MIN_RATE()) / 2;
        assertApproxEqRel(rateHalf, expectedHalfRate, 0.01e18, "40% utilization should be halfway between min and vertex");
    }
    ...
    function testInterestRateAfterKink() public {
        ...
        assertEq(rateKink, interestRate.VERTEX_RATE(), "80% utilization should give VERTEX_RATE");
        assertEq(rateMax, interestRate.MAX_RATE(), "100% utilization should give MAX_RATE");
        
        // 90% utilization should be halfway between vertex and max rates  
        uint256 expectedNinetyRate = interestRate.VERTEX_RATE() + (interestRate.MAX_RATE() - interestRate.VERTEX_RATE()) / 2;
        assertApproxEqRel(rateNinety, expectedNinetyRate, 0.01e18, "90% utilization should be halfway between vertex and max");
    }
    ...
    function testInterestRateOverUtilization() public {
        ...
        assertEq(rateOver, rateMax, "Over 100% utilization should be capped at max rate");
    }
    ...
    function testAPRConversion() public {
        ...
        assertTrue(perSecondRate > 0, "Should produce non-zero per-second rate");
    ...
    function testCompoundInterest() public {
        ...
        uint256 newPrincipal = interestRate.calculateCompoundInterest(principal, rate, oneYear);
        ...
        assertTrue(newPrincipal < principal * 100, "Interest rate should be reasonable");
    }
    ...
    function testZeroInterest() public {
        ...
        uint256 result1 = interestRate.calculateCompoundInterest(principal, interestRate.MIN_RATE(), 0);
        assertEq(result1, principal, "Zero time should give no interest");
        
        // Zero rate should give no interest  
        uint256 result2 = interestRate.calculateCompoundInterest(principal, 0, 3600);
